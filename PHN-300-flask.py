...
def till_display_proceesing(file_address,data):
    ...
    ####### INPUT
    delta_freq=data['delta_freq']       #input example [0.5,4]
    theta_freq=data['theta_freq']       #inuput example[4,7]
    alpha_freq=data['alpha_freq']       #input example [8,12]
    beta_freq=data['beta_freq']        #input example[12,16]
    gamma_freq=data['gamma_freq']       # input example  [13,30]
    ...
    initial_datapoint=data['initial_datapoint']          #example 0     
    final_datapoint=data['final_datapoint']            #example 4097
    ...
    plt.savefig(file_address[-1]+'.png') 
...
###########################################################
#flask dispkay 2
def till_classification_processing(file_address,data):
    ...
    ### Filtering EEG data=> FIR or IIR filters or bandpass filters or None
    ##########################################
    sampling_freq= data['sampling_rate'] # INPUT
    ##########################################
    apply_filter=data['apply_filter']  #option ['yes','no']
    ############### INPUT
    if(apply_filter=='yes'):
    ...
    window_size=data['window_size']      #example 4097 
    ...
    embedded_dim=data['embedded_dim'] #example [5] 
...
#######################################
def till_classification(data):       
        file_address0 =data['address_A'] #example r'D:\EEG\EEG dataset\Epilepic Dataset\A'  # INPUT
        file_address1 =data['address_B'] #example r'D:\EEG\EEG dataset\Epilepic Dataset\E'  # INPUT
                
        # Till now a def file which take two different classes as input
              
        #### Clasification Task
        X_class0= pd.DataFrame(till_classification_processing(file_address0,data))
        
        X_class1= pd.DataFrame(till_classification_processing(file_address1,data))
        ...
        feat_selection_method=data['feat_selection_method']      #example 'SelectKBest'
        precentage_or_value =data['percentage_or_value']  #example  20 
        ...
        cv_fold =data['cv_fold']     #example 5
        k_fold_options= [KFold,StratifiedKFold] # Input in term of Index
        k_fold_index=data['k_fold_index']   # example 1
        #######################
        ...
        input_classifier_index=data['classifier_index'] #example 0 
